<script>
    ...
    import {context,drawing,path,pen,names,sidebartool,shapetool,colortool,colorPallete} from './stores/store.js';
    import {drawBoard,eraseBoard,shapeBoard} from './utils/socketdraw.js';
    ...
    onMount(()=>{
        ...
        socket.addEventListener("open",()=>{ 
            ...
            drawing.subscribe((value)=>{
                if (value != undefined){
                    console.log($pen);
                    if (!value & $path.m.length > 0 & $pen.type != 1){
                        const mes = {a:"p",
                                    d : $path,
                                    p : {
                                        t:$pen.type,
                        ...
                        console.log("sending data",mes,$drawing);
        ...
        socket.addEventListener("message",(ev)=>{
            ...
            if(data.a === "added"){
                if(data.r === "you"){
                    ...
                    names.update((p)=>{
                        p[data.id] = {name : data.n,id:data.id,canvas: board,ctx: board.getContext('2d')};
                        return p;
                    })
                }
                ...
                connected = false;
                 ...
                 data.l.forEach((value)=>{
                    if(!names[value.id]){
                        const board = createBoard();
                        names.update((p)=>{
                            p[value.id] = {name : value.name,id:value.id,canvas: board,ctx: board.getContext('2d')};
                            return p;
                        });
                    }
                 })
                ...
                if(data.p.t == 2 | data.p.t == 3 ){
                    console.log("draw wala aaya");
                    drawBoard(data.d,$names[data.i].canvas,$names[data.i].ctx,data.p.s, $colorPallete[parseInt(data.p.c) -1],$context);
                }else if(data.p.t == 4){
                    console.log("erase wala aaya");
                    eraseBoard(data.d,$context,$names);
                }else if(data.p.t == 5){
                    console.log("shape wala aaya");
                    shapeBoard(data.p.m,data.d,$names[data.i].canvas,$names[data.i].ctx,$colorPallete[parseInt(data.p.c) -1],data.p.s,$context);
                }
    ...
    const toggleShape =(e) =>{
        const stoolbar = document.getElementsByClassName("shapeToolbar")[0]
        if(toogleshape){
            stoolbar.style.display = "none";
            toogleshape = !toogleshape
            return;
        }
        toogleshape = !toogleshape;
        stoolbar.style.display = "flex";
    }
    ...
    const toggleColor =(e) =>{
        const stoolbar = document.getElementsByClassName("colorToolbar")[0]
        if(tooglecolor){
            stoolbar.style.display = "none";
...
{:else }
    ...
    {#if connected}
        ...
        <div class="main">  
            ...
            <div class="toolbar">
                <Newicon id="1" store={sidebartool} checker={[checkerShape,checkerColor]} type="sd">
                ...
                <Newicon id="2" store={sidebartool} checker={[checkerShape,checkerColor]} type="sd" >
                ...
                <Newicon id="3" store={sidebartool} checker={[checkerShape,checkerColor]} type="sd" >
                ...
                <Newicon id="4" store={sidebartool} checker={[checkerShape,checkerColor]}  type="sd" >
                ...
                <Newicon id="5" store={sidebartool} func={toggleShape} checker={[checkerColor]} type="sd" >
                ...
                <Newicon id="6" store={sidebartool} func={toggleColor} checker={[checkerShape]} type="sd" >
                    <div  style={`border-radius:15px;background-color:${$colorPallete[parseInt($pen.color) -1]};width:30px;height:30px;`}></div>
                </Newicon>

                <Newicon id="7" func={showOverlay} store={sidebartool} checker={[checkerShape,checkerColor]}  type="sd" >
            ...
            <div class="shapeToolbar">
                <Newicon id="1" store={shapetool}  type="s">
                    <svg width="30" height="30" viewBox="0 0 24 24"><path d="M21.71,3.29a1,1,0,0,0-1.42,0l-18,18a1,1,0,0,0,0,1.42,1,1,0,0,0,1.42,0l18-18A1,1,0,0,0,21.71,3.29Z"/></svg>
                </Newicon>

                <Newicon id="2" store={shapetool}  type="s" >
                    <svg viewBox="0 0 256 256" height="30" width="30"><rect fill="none" height="256" width="256"/><rect height="176" rx="16" width="208" x="24" y="40"/></svg>
                </Newicon>

                <Newicon id="3" store={shapetool}  type="s" >
                    <svg id="Layer_1_1_" style="enable-background:new 0 0 16 16;" width="30" height="30" viewBox="0 0 16 16" xml:space="preserve"><circle cx="8" cy="8" r="8"/></svg>
                </Newicon>

            </div>

            <div class="colorToolbar">
                <Newicon id="1" store={colortool}  type="c" >
                    <div  style="border-radius:15px;background-color:#000;width:30px;height:30px;"></div>
                </Newicon>

                <Newicon id="2" store={colortool}  type="c" >
                    <div  style="border-radius:15px;background-color:#ff0000;width:30px;height:30px;"></div>
                </Newicon>

                <Newicon id="3" store={colortool}  type="c" >
                    <div  style="border-radius:15px;background-color:#ffee00;width:30px;height:30px;"></div>
                </Newicon>

                <Newicon id="4" store={colortool}  type="c" >
                    <div  style="border-radius:15px;background-color:#0ae415;width:30px;height:30px;"></div>
                </Newicon>

                <Newicon id="5" store={colortool}  type="c" >
                    <div  style="border-radius:15px;background-color:#0330fc;width:30px;height:30px;"></div>
                </Newicon>

                <Newicon id="6" store={colortool}  type="c" >
                    <div  style="border-radius:15px;background-color:#fc03ef;width:30px;height:30px;"></div>
                </Newicon>
